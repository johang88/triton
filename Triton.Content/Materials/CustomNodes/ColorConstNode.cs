...
namespace Triton.Content.Materials.CustomNodes
{
	public class ColorConstNode : NodeGraphNode
	{
		...
		{
			Setup();
			Value = StringConverter.Parse<Vector4>(p_TreeNode.m_attributes["Value"]);
		}
		...
		public override NodeGraphData Process(int connectorIndex)
		{
			...
			else
			{
				...
				var statements = new List<string>()
				{
					string.Format("vec4 {0} = vec4({1}, {2}, {3}, {4})", outputVar, StringConverter.ToString(scaledValue.X), StringConverter.ToString(scaledValue.Y), StringConverter.ToString(scaledValue.Z), StringConverter.ToString(scaledValue.W)),
				};
